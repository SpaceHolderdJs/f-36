...
class FormConstructor {
  constructor(name, fieldsData, options) {
    ...
    if (!fieldsData) {
      throw new Error("Fields Data is required to create form");
    }
    this.name = name;
  ...
  init(fieldsData) {
    ...
    if (onSubmit) {
      ...
      form.onsubmit = (event) => {
        ...
        // [Note]:update form data before we send to you
        this.updateFormData();
  ...
  updateFormData() {
    for (const fieldElementName in this.fieldsElements) {
      this.formData.update(
        fieldElementName,
        this.fieldsElementsName[fieldElementName].value
      );
    }
  }
}
...
// [Note]: FormData class (for the data of the form)b
